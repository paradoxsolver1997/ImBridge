...
from src.utils.commons import confirm_overwrite, check_tool
...
def trace_image(in_path, out_dir, logger: Optional[Logger] = None) -> Optional[str]:
    ...
    if confirm_dir_existence(out_dir):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if in_fmt != '.bmp':
                ...
                bmp_path = grayscale_image(temp_bmp, tmp_dir, binarize=True, logger=logger)
            else:
                bmp_path = grayscale_image(in_path, tmp_dir, binarize=True, logger=logger)
            out_path = trace_bmp_to_svg(bmp_path, out_dir, logger=logger)
        logger.info(f'Traced {os.path.basename(in_path)} to {os.path.basename(out_path)} successfully.') if logger else None
        return out_path


def grayscale_image(
    in_path: str, out_dir: str, binarize: bool = False, logger: Optional[Logger] = None
) -> Optional[str]:
    """
